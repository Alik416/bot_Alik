...
#обработчик списка стран апостилиус
...
contacts_markup = types.ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=True)
contacts_markup.row(types.KeyboardButton("Геопозиция офиса на улице Советов, 42"),
                    types.KeyboardButton("Геопозиция офиса на улице Мира, 37(рядом с нотариальной конторой)"))
...
def send_office_location_sovetov(message):
    ...
    bot.send_message(message.chat.id, "Ссылка на Google Maps для офиса на улице Советов, 42:",
                     reply_markup=InlineKeyboardMarkup().add(
                         InlineKeyboardButton(text="Открыть Google Maps", url=google_maps_url_sovetov)))



@bot.message_handler(func=lambda message: message.text == "Геопозиция офиса на улице Мира, 37(рядом с нотариальной конторой)")
def send_office_location_mira(message):
    google_maps_url_mira = "https://maps.app.goo.gl/9Bp41YqQBUsYAbHo6"  # Замените на фактическую ссылку
    bot.send_message(message.chat.id, "Ссылка на Google Maps для офиса на улице Мира, 37(рядом с нотариальной конторой):",
                     reply_markup=InlineKeyboardMarkup().add(
                         InlineKeyboardButton(text="Открыть Google Maps", url=google_maps_url_mira)))
    send_main_menu_with_return_button(message)


def send_main_menu_with_return_button(message):
    return_button_markup = types.ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=True)
    return_button_markup.row(types.KeyboardButton("Вернуться на главное меню"))
